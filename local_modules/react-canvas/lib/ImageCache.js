...
var _lruInstancePool = {
  ...
  popLeastUsed: function () {
    ...
    if( minUsed.hash ){
      return this.removeElement(minUsed.hash).data;
    }
...
var _circularInstancePool = {
  _pool: [],
  _refs: {},
  length: 0,
  push: function (path, data) {
    this._refs[path] = data;
  ...
  popLeastUsed: function () {
    var path = this._pool.shift();
    var element = this._refs[path];
    delete this._refs[path];
    this.length--;
    return element;
  }
}
//.#_instancePool = _lruInstancePool
var _instancePool = _circularInstancePool;
